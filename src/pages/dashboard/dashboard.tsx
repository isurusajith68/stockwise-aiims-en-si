...
function StatCard({ title, value, trend, trend_direction, icon: Icon, alert = false }: StatCardProps) {
  return (
    <Card className={alert ? "border-red-200 bg-red-50 dark:bg-red-950/10 dark:border-red-800" : ""}>
      <CardContent className="p-6">
        <div className="flex items-center justify-between">
          ...
          <div className={`rounded-full p-2 ${alert ? 'bg-red-100 text-red-600 dark:bg-red-900/20 dark:text-red-400' : 'bg-primary/10 text-primary'}`}>
            <Icon className="h-5 w-5" />
          </div>
        </div>
        <div className="mt-4 flex items-center">
          {trend_direction === 'up' ? (
            <TrendingUp className={`h-4 w-4 mr-1 ${alert ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`} />
          ) : (
            <TrendingDown className="h-4 w-4 mr-1 text-green-600 dark:text-green-400" />
          )}
          <span className={`text-sm font-medium ${trend_direction === 'up' ? alert ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400' : 'text-green-600 dark:text-green-400'}`}>
            {trend}
          </span>
        </div>
      </CardContent>
