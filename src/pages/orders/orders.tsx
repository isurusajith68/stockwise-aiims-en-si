...
const formatStatus = (status: string) => {
  switch (status) {
    case 'pending': return { label: 'Pending', variant: 'secondary' as const, icon: FileText };
    case 'confirmed': return { label: 'Confirmed', variant: 'outline' as const, icon: CheckCircle2 };
    case 'shipped': return { label: 'Shipped', variant: 'default' as const, icon: Truck };
    case 'delivered': return { label: 'Delivered', variant: 'success' as const, icon: PackageCheck };
    case 'cancelled': return { label: 'Cancelled', variant: 'destructive' as const, icon: XCircle };
    default: return { label: status, variant: 'outline' as const, icon: FileText };
  }
};
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
        <h1 className="text-3xl font-bold tracking-tight">{translations.orders}</h1>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-center">{translations.items}</TableHead>
            ...
            <TableBody>
              {filteredOrders.length === 0 ? (
                ...
                filteredOrders.map((order) => {
                  const { label, variant, icon: StatusIcon } = formatStatus(order.status);
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem>{translations.viewOrder}</DropdownMenuItem>
                            {order.status === 'pending' && (
                              <>
                                <DropdownMenuItem>{translations.editOrder}</DropdownMenuItem>
                                <DropdownMenuSeparator />
      ...
      {isAddOrderOpen && (
        <OrderForm onClose={() => setIsAddOrderOpen(false)} />
      )}
    </div>
  );
}